...
ANSIBLE_CFG_FILE="$(pwd)/ansible/worker/hosts/ansible.cfg"

# setup hosts
WORKER_HOST_IP=$(grep ^host HOSTS_CONF_FILE | awk -F "=" '{print $2}')
...
ansible-playbook -vvv ansible/worker/deploy.yml